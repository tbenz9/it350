...
class DeviceController extends AppBaseController
{
	...
	public function Query()
	{
		try
		{
			...
			if ($filter) $criteria->AddFilter(
				new CriteriaFilter('DevicesId,SerialNumber,Model,DevicesType,EmployeesId'
				, '%'.$filter.'%')
			);
	...
	public function Read()
	{
		try
		{
			$pk = $this->GetRouter()->GetUrlParam('devicesId');
	...
	public function Create()
	{
		try
		{
			...
			$device->DevicesId = $this->SafeGetVal($json, 'devicesId');
			$device->SerialNumber = $this->SafeGetVal($json, 'serialNumber');
			$device->Model = $this->SafeGetVal($json, 'model');
			$device->DevicesType = $this->SafeGetVal($json, 'devicesType');
			$device->EmployeesId = $this->SafeGetVal($json, 'employeesId');
	...
	public function Update()
	{
		try
		{
			...
			// $device->DevicesId = $this->SafeGetVal($json, 'devicesId', $device->DevicesId);

			$device->SerialNumber = $this->SafeGetVal($json, 'serialNumber', $device->SerialNumber);
			$device->Model = $this->SafeGetVal($json, 'model', $device->Model);
			$device->DevicesType = $this->SafeGetVal($json, 'devicesType', $device->DevicesType);
			$device->EmployeesId = $this->SafeGetVal($json, 'employeesId', $device->EmployeesId);
